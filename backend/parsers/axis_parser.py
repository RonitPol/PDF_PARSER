# backend/parsers/axis_parser.py
...
class AxisParser(BaseCreditCardParser):
    ...
    def get_card_number_last4(self):
        ...
        # Check for Axis sample pattern
        if 'XXXX XXXX XXXX 8765' in self.text:
            return "8765"
        return "Not Found"
    ...
    def get_billing_period(self):
        patterns = [
            ...
            r'Bill Cycle:\s*(\d{2}-\w{3}-\d{4})\s*to\s*(\d{2}-\w{3}-\d{4})'
        ]
        for pattern in patterns:
            match = re.search(pattern, self.text, re.IGNORECASE)
            if match:
                return f"{match.group(1)} to {match.group(2)}"
        # Check for Axis sample content
